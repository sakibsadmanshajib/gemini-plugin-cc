# Legacy `plugins/gemini/` plugin

> **NOTE:** This document describes the **legacy gemini-driving plugin** (host = Claude Code or Codex CLI; drives Gemini via `gemini --acp` broker-shared multi-turn ACP). It predates the [cross-pollination model](plugins.md) and is preserved for backward compatibility. The plugin's commands continue to work; new install instructions point at the same source tree.
>
> For the new cross-pollination model — separate `plugins/claude/`, `plugins/codex/`, `plugins/gemini/` plugins each driving the OTHER backends — start at [`docs/plugins.md`](plugins.md) and the top-level [`README.md`](../README.md).
>
> The full original README content for this plugin follows below, unchanged.
