...
- **Auto-start** — `lib/server/auto-start.mjs::autoStartFacade` spawns the daemon when a slash-command finds no live manifest. proper-lockfile serializes both the spawn AND the circuit-breaker read/append (V1 round-12 R1 fix). The daemon child's stdio is redirected to `auto-start.log` under XDG state — spawn-level failures (ENOENT, non-zero exit) are kept in-process as the captured error and surfaced via the thrown error message, not written to the log.
...
- **Circuit breaker** — `$XDG_STATE_HOME/artagon-agent-cli-plugin/auto-start-failures.json` tracks daemon spawn failures in a 5-minute rolling window. Three failures → next `autoStartFacade` refuses with an actionable message. A successful daemon boot (manifest appears + no exit error) clears the log. Stale entries are dropped from the in-memory rolling window on read; the on-disk file is rewritten on the next append or clear.
...
- **/admin/status** — bearer-gated (or unauthed when `apiKey` is unset) GET endpoint reporting pid, uptime, per-supervisor health, and SQLite recorder stats. `lastError` is redacted to a closed `LastErrorCode` enum (see `lib/runners/streaming/types.mjs`) — never raw `err.message`, so spawn paths and auth hints stay in the daemon's stderr log. Unrecognized runner errors map to the explicit `"unknown"` bucket rather than passing through.
- **Machine-readable errors** — dispatcher facade-failure throws **on the wipe-eligible network codes** carry `.code: "FACADE_UNREACHABLE" | "FACADE_RACE_REPLACED" | "FACADE_CONN_RESET"`. Other failures (`AbortError` from per-call timeout, non-2xx HTTP responses, bare-rethrow on unknown error codes) propagate without an attached `.code` — downstream catch blocks should default-handle the no-code case.
...
- **`LastErrorCode` is the wire contract** — `/admin/status` exposes only the closed enum from `lib/runners/streaming/types.mjs`. The JSDoc + `checkJs:true` typecheck prevents accidentally emitting a non-union string, but new error shapes still fall through to the explicit `"unknown"` bucket — adding a real category requires updating the union, the classifier regex in `lib/runners/streaming/registry.mjs`, AND the table in `docs/openai-facade.md` together.
