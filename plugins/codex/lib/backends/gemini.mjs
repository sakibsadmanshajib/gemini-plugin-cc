...
import { createBrokerSocketTransport } from "../transport/broker-socket.mjs";
 ...
 *   brokerSocket: (endpoint: string, config?: BackendConfig) => ClientTransport
 * }} transports
...
export const geminiBackend = {
  ...
  transports: {
    ...
    /**
     * Connect to an existing broker via its Unix socket / named pipe.
     * Used for cross-cutting operations (e.g. session/cancel) where a
     * fresh subprocess wouldn't reach the live session.
     *
     * @param {string} endpoint
     * @param {BackendConfig} [_config]
     */
    brokerSocket(endpoint, _config = {}) {
      return createBrokerSocketTransport({ endpoint });
    }
  },
