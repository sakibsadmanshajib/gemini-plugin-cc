 ...
 *   GET  /v1/models/{id}              — Single-model retrieval (404 unknown)
 *   GET  /admin/status                — Operator health snapshot (K1)
 *   GET  /health                      — Liveness probe (always 200; auth-exempt)
 *   OPTIONS  *                        — CORS preflight when enabled
 *
 * Backend routing:
 ...
 *
 * Authentication:
 *   `--api-key sk-...` (single or comma-separated allowlist; or
 *   `$ARTAGON_FACADE_API_KEY`) gates `/v1/*` and `/admin/status` behind
 *   `Authorization: Bearer <key>` (constant-time compare). `/health` is
 *   always exempt (load-balancer probe). When `--api-key` is unset the
 *   facade is fully unauthed — safe with the default loopback bind, but
 *   if rebinding to 0.0.0.0 the operator MUST set --api-key. The
 *   /admin/status response's `lastError` is a redacted enum (closed
 *   `LastErrorCode` union from `lib/runners/streaming/types.mjs`) so an
 *   unauthed status read can't exfiltrate filesystem paths or auth hints.
 *
 * CORS:
 *   `--cors '*'` (or comma-separated origin allowlist; or
 *   `$ARTAGON_FACADE_CORS`). Off by default. Preflight (OPTIONS) is
 *   handled before auth so browsers can probe.
 *
 * Operator-facing observability:
 *   `/admin/status` returns `{pid, startedAt, uptimeMs, supervisors,
 *   stats, auth}` for daemon health-checks. See `docs/openai-facade.md`
 *   for the response shape + LastErrorCode enum table.
 */
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && req.url === "/v1/chat/completions") {
        ...
        const cwdHeaderRaw = /** @type {string | undefined} */ (req.headers["x-artagon-cwd"]);
        const cwdHeader = Array.isArray(cwdHeaderRaw) ? cwdHeaderRaw[0] : cwdHeaderRaw;
        /** @type {string | null} */
        let requestCwd = null;
