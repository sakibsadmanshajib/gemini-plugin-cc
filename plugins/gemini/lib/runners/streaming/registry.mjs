...
export function getSupervisorStatuses() {
  ...
  for (const [backend, sup] of supervisors.entries()) {
    ...
    try {
      ...
      // Introspection itself threw — the supervisor object is unsafe
      // to read further. Mark dead + introspect_failed so operators
      // see something rather than an empty array.
      health = "dead";
