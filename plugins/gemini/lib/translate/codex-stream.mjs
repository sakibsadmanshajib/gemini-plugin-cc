...
function translateTurnCompleted(event) {
  ...
  // Codex `exec --json` reports the model id either at top level or
  // under message.model. Capture whichever is present so the cost
  // record can hit per-model pricing rates.
  const ev = /** @type {any} */ (event);
  const model = ev.model ?? ev.message?.model ?? null;
