 ...
 * }>} DispatchPolicy
 *
 * **H6:** post-Step-5, `dispatch.streaming` is no longer read for
 * routing (the only paths are facade or streaming, and absent-facade
 * implies streaming). The field is preserved as a documentation
 * carrier (operators set `--streaming` to be explicit) but does not
 * affect dispatch behavior. A future refactor can drop the field once
 * the slash-command's `dispatch.streaming === "default"` promotion in
 * `lib/cli/script-entry.mjs` is rewritten.
 *
 * @typedef {Readonly<{
 *   wireLogPath?: string,
 *   wireLogRaw?:  boolean,
...
function resolveDispatch(flags, env) {
  /** @type {{ streaming?: TriState, facade?: TriState }} */
  const out = {};
