 ...
 *   - "thread/start returned no thread id" → session_init_failed
...
/**
 * Codex app-server's `turn/start.params.effort` accepts:
 *   `none | minimal | low | medium | high | xhigh`
 * Anything else is rejected with `Invalid request: unknown variant`.
 *
 * We expose `max` as a portable cross-backend alias for "highest effort
 * this backend supports". Codex's highest is `xhigh`, so map it here.
 * Already-valid codex values pass through unchanged; unknown values
 * fall back to `xhigh` (the safest "do more work" default).
 *
 * @param {string | undefined} effort
 * @returns {"none" | "minimal" | "low" | "medium" | "high" | "xhigh"}
 */
      ...
      try {
        ...
        const work = (async () => {
          ...
          const startResp = await /** @type {any} */ (client).request("turn/start", {
            ...
            // V3 (2026-05-12): forward the reasoning-effort knob to
            // codex app-server. Codex's vocabulary is
            // `none|minimal|low|medium|high|xhigh`; the portable
            // alias `max` maps to `xhigh` (its highest level). The
            // default is the highest level so daemon-served codex
            // turns use full reasoning budget unless callers ask
            // for less via `reasoning_effort`.
            effort: normalizeCodexEffort(turnOpts.effort)
          });
