...
const GEMINI_SOURCE = fs.readFileSync(
  path.join(ROOT, "plugins/gemini/scripts/lib/gemini.mjs"),
  "utf8"
);
    ...
    [
      {
        ...
        params: {
          sessionId: "s",
          update: { sessionUpdate: "tool_call", toolName: "read\u001b[2J_file\n[done] forged" }
        }
      },
      {
        method: "session/update",
...
test("simulateNotificationDispatch fires tool_call and file_change as stream events", () => {
  const { events } = gemini.simulateNotificationDispatch([
    {
      method: "session/update",
      params: { sessionId: "s", update: { sessionUpdate: "tool_call", toolName: "read_file" } }
    },
    {
      method: "session/update",
      params: {
        sessionId: "s",
        update: { sessionUpdate: "file_change", path: "a.mjs", action: "write" }
      }
    }
  ]);
  assert.deepEqual(
