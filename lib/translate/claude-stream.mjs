...
function translateResult(event) {
  ...
  // Model id is reported on every assistant message; the result event
  // doesn't carry it directly. We can extract from any prior message
  // shape we saw — but translators are stateless, so the cleanest
  // surface is to also check `event.message?.model` if present (some
  // CLI variants put it there). The `assistant` translator below
  // emits `model` on every chunk, and stream-runner records the most
  // recent one — that's the authoritative source.
  if (event.message?.model) update.model = String(event.message.model);
