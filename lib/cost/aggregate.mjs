...
export function summarizeCostRecords(records, options = {}) {
  ...
  for (const rec of records) {
    ...
    // Estimate USD for this turn. Model id isn't always recorded by
    // the runners; pricing falls back to per-backend defaults when
    // model is unknown. See lib/cost/pricing.mjs for the table.
    const model = /** @type {any} */ (rec).model ?? null;
    const turnUsd = estimateUsd(rec.backend, model, usage, {
