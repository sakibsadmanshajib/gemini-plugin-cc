...
export function appendCostRecord(record, options = {}) {
  ...
  // Step 1.5: also write to SQLite when context.cost.sqlitePath is set
  // (typically by the facade daemon at boot). The SQLite recorder is
  // an independent best-effort surface; JSONL stays as the canonical
  // append-only store for ad-hoc + the slash-command path.
  if (options.context?.cost?.sqlitePath) {
    // Lazy import — keep node:sqlite out of the load graph for callers
    // that never touch it (e.g. slash-command scripts without a daemon).
    import("./sqlite-recorder.mjs")
      .then(({ insertTurnStat }) => insertTurnStat(full, options))
      .catch(() => {
        // best-effort; failures already warn via the sqlite-recorder's
        // own warnedOnce gate.
      });
  }
}
