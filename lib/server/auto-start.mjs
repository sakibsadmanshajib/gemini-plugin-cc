 ...
 *
 * Flow:
 *   1. Sweep stale `*.tomb.*` files older than 1 hour (T1, round-13):
 *      a SIGKILL of `compareAndDeleteManifest` between its rename and
 *      cleanup can leak a tombstone in the manifest directory.
 *   2. Acquire an exclusive lock on $XDG_STATE_HOME/artagon-agent-cli-
 *      plugin/auto-start.lock so two concurrent slash-commands don't
 *      both spawn the daemon. The lock also serializes the circuit-
 *      breaker read/append (R1, round-12) so two concurrent retry
 *      attempts can't both miss recording their failures.
 *   3. Re-check the manifest under the lock. If another process won
 *      the race and a daemon is up, return — we don't spawn a second.
 *   4. Read the circuit breaker (Q4, round-11): if 3+ failures within
 *      the 5-minute rolling window, refuse to spawn and tell the
 *      operator to fix the underlying issue (link to auto-start.log).
 *   5. Spawn `artagon-openai-server` detached + unref so the daemon
 *      outlives the slash-command. The daemon's stdout+stderr are
 *      redirected to `auto-start.log` under XDG state (J3) so the
 *      daemon's listen-fail / auth-fail / port-in-use messages are
 *      diagnosable. The child's own dup'd fd handle survives our
 *      `closeSync(logFd)` call.
 *   6. Poll the manifest until it appears (or timeout). The daemon
 *      writes the manifest after binding its listening socket; once
 *      we see it, we can fetch().
 *   7. On successful daemon boot (manifest appeared AND no exit error
 *      since spawn), clear the breaker's failure log. On any failure
 *      path, append a timestamp to the breaker.
 *   8. Release the lock.
 *
 * Failure modes (loud, not silent):
 *   - Lock acquire times out → throw with the lock path so the operator
 *     can `rm` a stale lock manually.
 *   - Circuit breaker tripped (3+ failures in 5 min) → throw with an
 *     actionable `rm <failure-log-path>` reset instruction.
 *   - Spawn fails (ENOENT / EACCES / non-zero exit) → throw with the
 *     captured `spawnError` from `child.on("error")` / `child.on("exit")`.
 *   - Daemon wrote manifest then crashed (O1, round-10) → throw with
 *     the "wrote manifest then crashed" message, log file linked.
