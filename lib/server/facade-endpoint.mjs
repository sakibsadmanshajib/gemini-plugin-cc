...
/**
 * Is the manifest file a regular owned-by-current-uid file?
 *
 * U1 (round-13 B2 defense-in-depth): use `lstatSync` so a symlink at
 * the manifest path is REJECTED rather than followed. If an attacker
 * with write access to `$XDG_STATE_HOME` placed the manifest as a
 * symlink to a foreign-owned file, the previous `statSync` would
 * follow the symlink and report the target's uid — which is correct
 * by accident (target is foreign-owned → ownership fails → caller
 * bails). `lstatSync` makes the refusal explicit: we will not even
 * consider a symlink to be a valid manifest, period.
 *
 * @param {string} file
 * @returns {boolean}
 */
