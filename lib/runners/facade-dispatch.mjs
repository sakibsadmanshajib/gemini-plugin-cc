 ...
 *
 * Default routing in slash-commands: when the slash-command boundary
 * (`lib/cli/script-entry.mjs`) finds a live manifest at
 * `$XDG_STATE_HOME/artagon-agent-cli-plugin/facade-endpoint.json`, or
 * when `autoStartFacade` successfully spawns one, the dispatcher
 * routes through this function. Operators can opt out per-call via
 * `--no-facade` (which maps to `context.dispatch.facade === "off"`)
 * or opt in unconditionally via `--facade` / `ARTAGON_USE_FACADE=1`.
 * The env var is read once at the slash-command boundary in
 * `lib/agent-context.mjs` (Phase 4 of the AgentContext refactor);
 * lib code never reads it directly.
 *
 * Auth: when the facade has --auto-key enabled, the manifest contains
 * the retrieve-command. The boundary in `lib/cli/script-entry.mjs`
 * auto-resolves the key from the 0o600 key file (when manifest.autoKey.store
 * is "file") or prints an actionable retrieve-command hint (when it's
 * "keychain"). Dispatcher never reads keys from disk directly.
 *
 * Failure mode: any error → reject. The caller propagates the
 * rejection up to the slash-command boundary; there is no fallback
 * chain post-Step 5 (see `lib/runners/dispatch.mjs` header for the
 * "why no fallback" rationale).
 */
...
import { compareAndDeleteManifest, readManifest } from "#lib/server/facade-endpoint.mjs";
 ...
 *
 * Streaming behavior: when `options.onUpdate` is set the request is
 * sent with `stream: true` + `Accept: text/event-stream` and the
 * response is parsed via `consumeSseStream` (each delta chunk fires
 * `onUpdate`). When `onUpdate` is unset, the request is non-streaming
 * and the full JSON response is mapped once at completion.
 *
 * Failure modes (see also dispatch.mjs::shouldUseFacade):
 *   - `readManifest` returns null → throw "no running facade found"
 *   - `fetch` rejects with `ECONNREFUSED`/`ENOTFOUND`/`EHOSTUNREACH`/
 *     `ENETUNREACH` → `compareAndDeleteManifest` atomically wipes the
 *     stale manifest if pid+port match; throw with `.code = "FACADE_UNREACHABLE"`
 *     (or `"FACADE_RACE_REPLACED"` when another process won the rename)
 *   - `fetch` rejects with `ECONNRESET` → throw with
 *     `.code = "FACADE_CONN_RESET"` (manifest is NOT wiped; the daemon
 *     may be serving other clients)
 *   - non-2xx response → throw with the status; cost-record gets
 *     `model` + `usage` parsed from the body when present (N2)
 *
 ...
 *   `context.session.action` shapes the `X-Artagon-Session` /
 *   `X-Artagon-New-Session` headers forwarded to the daemon.
 *   `context.cwd` (or `options.cwd`) forwards as `X-Artagon-Cwd`.
...
export async function runViaFacade(backend, options, context) {
  ...
  try {
    ...
    if (
      ...
      // S1 (round-13): race-safe wipe via atomic rename + verify +
      // restore. compareAndDeleteManifest atomically claims whatever's
      // at the manifest path right now (via rename to a unique
      // tombstone), verifies the contents match the expected pid+port,
      // and either commits the delete OR restores the file when we
      // tombstoned a fresher manifest. Closes the M2 TOCTOU window
      // where the previous read-then-delete path could wipe a healthy
      // daemon's discovery file.
      const env = context?.env ?? options.env ?? process.env;
      const result = compareAndDeleteManifest({ pid: manifest.pid, port: manifest.port }, env);
      if (result.committed) {
        // Q5 (round-11): attach a machine-readable .code so downstream
        // catch blocks can switch on the error class without substring-
        // matching the prose. Three classes: FACADE_UNREACHABLE,
        // FACADE_RACE_REPLACED, FACADE_CONN_RESET.
        const e = new Error(
        ...
        /** @type {any} */ (e).code = "FACADE_UNREACHABLE";
        throw e;
      }
      // Mismatch / already-gone — someone else already replaced or
      // removed the stale entry, or we tombstoned a different daemon's
      // (fresher) manifest and restored it. The hint guides the
      // operator to retry; the next call will read whatever's now at
      // the manifest path.
      const e = new Error(
        `runViaFacade: cannot reach artagon-openai-server at http://${manifest.host}:${manifest.port} (${code}). ` +
          `Another process has already refreshed or removed the manifest (${result.reason ?? "race"}) — retry the command.`
      );
      /** @type {any} */ (e).code = "FACADE_RACE_REPLACED";
      throw e;
    }
    if (code === "ECONNRESET") {
      ...
      /** @type {any} */ (e).code = "FACADE_CONN_RESET";
      throw e;
    }
    throw err;
  }
