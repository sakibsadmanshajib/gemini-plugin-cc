...
export async function runViaFacade(backend, options, context) {
  ...
  try {
    ...
    if (
      ...
      // may have already replaced this manifest with a freshly-spawned
      // daemon's entry. Only unlink when the on-disk manifest STILL
      // points at the pid+port we just failed against — otherwise we'd
      // delete a healthy daemon's discovery file and break every other
      // concurrent client.
      const env = context?.env ?? options.env ?? process.env;
      const currentManifest = readManifest(env);
      if (
        currentManifest &&
        currentManifest.pid === manifest.pid &&
        currentManifest.port === manifest.port
      ) {
        deleteManifest(env);
      ...
      // Someone else already replaced or removed the stale entry.
      // Don't unlink — just surface the fatal-network error with the
      // hint that a retry will pick up whatever exists now.
      const e = new Error(
        `runViaFacade: cannot reach artagon-openai-server at http://${manifest.host}:${manifest.port} (${code}). ` +
          "Another process has already refreshed or removed the manifest — retry the command."
      );
