 ...
 *   Per-session config: dispatch policy, logging policy, cost policy,
 *   facade policy, plus session-shaped cwd / env / model / timeoutMs.
 *
 *   PRECEDENCE for the fields that exist on both:
 *
 *     context.cwd        wins over options.cwd
 *     context.env        wins over options.env
 *     context.model      wins over options.model      (turn-level `modelOverride` deferred)
 *     context.timeoutMs  wins over options.timeoutMs
 *
 *   Callers SHOULD migrate to passing these via `context` only;
 *   `options` is the legacy path retained for pre-Phase-3 callers.
 *   When both are set with the same value, behavior is identical
 *   (the `??` chains collapse). When they disagree, the dispatcher
 *   silently picks context — which is the documented contract but
 *   also the "two-source-of-truth" wart flagged by the v2 plan's
 *   type-design pass. A future refactor (Phase 5+) can drop the
 *   options-side fields entirely; until then, prefer context.
 *
 *   Dispatch policy (`context.dispatch.streaming` / `facade` /
 *   `broker`) takes precedence over the equivalent `options.use*` /
 *   `options.disable*` booleans, which in turn supersede the
 *   (Phase-4-removed) `process.env.ARTAGON_*` fallback.
 *
 *   `context` is threaded through to runners + transport + recorder
 *   so observability + cost knobs come from a single source of truth
 *   instead of via env vars.
 * @returns {Promise<TurnResult>}
 */
export function runStatelessTurn(backendName, options, context) {
  // Boundary guard: session policy is only honored by paths that wire
  // it to the agent — today that means the streaming runner OR the
  // facade (which forwards X-Artagon-Session/-New-Session headers to a
  // daemon that runs its OWN streaming runner internally). Cold-start
  // runners ignore context.session, so we fail loudly at the boundary
  // rather than silently produce a fresh conversation when the caller
  // asked to resume one (F2).
  const sessionAction = context?.session?.action;
  const sessionRequest = sessionAction === "fresh" || sessionAction === "resume";
  if (
    sessionRequest &&
    !shouldUseStreaming(options, context) &&
    !shouldUseFacade(options, context)
  ) {
    return Promise.reject(
      new Error(
        "runStatelessTurn: --session/--new-session requires streaming or facade " +
          `(got dispatch.streaming=${JSON.stringify(context?.dispatch?.streaming ?? "default")}, ` +
          `dispatch.facade=${JSON.stringify(context?.dispatch?.facade ?? "default")}). ` +
          "Pass --streaming or --facade, or remove the session flag."
      )
    );
  }
