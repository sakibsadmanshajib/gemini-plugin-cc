      ...
      try {
        ...
        switch (context?.session?.action ?? "reuse") {
          case "fresh": {
            ...
            // New session has no model applied yet — reset so the
            // per-turn applySessionModel below issues set_model.
            appliedModel = null;
            break;
          }
          case "resume": {
            ...
            // Resumed session: its server-side model selection is
            // whatever the prior owner left; force a reapply so this
            // session's model is well-defined.
            appliedModel = null;
            break;
          }
          default:
