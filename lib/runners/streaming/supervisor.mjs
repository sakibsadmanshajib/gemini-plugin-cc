 ...
 *
 * **Error-message contract (round-16 lock-in).** The "supervisor:
 * runner is dead (max restarts exceeded)" string thrown when the
 * restart budget exhausts is byte-exact-locked in
 * `tests/unit/streaming-registry.test.mjs` — it maps to the
 * `restart_budget_exhausted` LastErrorCode bucket. Rewording it
 * (e.g. "runner has died after max restarts") will fail the
 * specific lock-in test; update the matching test case in
