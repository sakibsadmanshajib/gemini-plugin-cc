...
export function createAuditMiddleware(userConfig = {}) {
  ...
  // security-relevant context (CodeQL js/insecure-randomness). The
  // audit log keys directories by sessionId, so a predictable id is
  // a real risk: an attacker who can guess session ids could probe for
  // (or write to, if filesystem perms slip) other sessions' logs.
  const sessionId =
  ...
  function ensureFd() {
    ...
    try {
      ...
      // Include the underlying error so an operator seeing "auditing disabled"
      // can distinguish EACCES (perms), ENOSPC (full disk), ENOENT (bad path),
      // EROFS (read-only fs), etc., without re-running with a debugger.
      const message = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[audit] failed to open log; auditing disabled — ${message}\n`);
    }
    return fd;
  }

  // Audit-write failures are silent by design (best-effort), but the FIRST
  // failure surfaces to stderr so operators see something rather than a
  // mysteriously empty audit log. Subsequent failures stay silent to avoid
  // log spam — they're almost always the same root cause repeating.
  let writeFailureLogged = false;

  /**
   * @param {"request"|"response"|"notify"|"inbound"|"health"} kind
  ...
  function record(kind, payload) {
    ...
    try {
      ...
      if (!writeFailureLogged) {
        ...
        const message = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[audit] write failed; subsequent failures silenced — ${message}\n`);
      }
    }
  }
