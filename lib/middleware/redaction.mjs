 ...
 *
 * Cross-layer invariant: the credential field-name set MUST stay aligned
 * with `lib/wire-log.mjs` REDACT_TOKENS and `lib/logger.mjs`
 * REDACTED_PATHS. Verified by `tests/unit/cross-layer-redaction.test.mjs`
 * which extracts each layer's name-set and asserts equivalence with
 * per-name diagnostics. A regression in any of the three lists fails
...
// equivalence across all three lists.
